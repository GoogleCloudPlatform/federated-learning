...
resource "null_resource" "copy_common_acm_content" {
  triggers = {
    ...
    source_contents_hash            = sha512(join("", [for f in fileset(local.acm_config_sync_common_content_source_path, "*") : filesha512("${local.acm_config_sync_common_content_source_path}/${f}")]))
